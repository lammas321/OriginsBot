...
{
    public sealed class CreatorsPagination : ComponentInteractionModule<ButtonInteractionContext>
    {
        public const int PerPage = 20;

        public static int Count
            => OriginDataService.Data.OrderedCreatorIds.Count;
        ...
        private static EmbedProperties GetEmbed(int page)
            => new()
            {
                Title = $"Origin Creators ({Count})",
                Description = $"- {string.Join("\n- ", OriginDataService.Data.OrderedCreatorIds.Skip(PerPage * page).Take(PerPage).Select(creatorId => OriginDataService.Data.UniqueCreatorNames[creatorId]))}",
...
#pragma warning disable IDE0028 // Simplify collection initialization
            => new([
                new ButtonProperties("creators_start", "<<", ButtonStyle.Primary) { Disabled = page == 0 },
                new ButtonProperties($"creators_back:{page}", "<", ButtonStyle.Primary) { Disabled = page == 0},
                new ButtonProperties($"creators_next:{page}", ">", ButtonStyle.Primary) { Disabled = page == MaxPage },
                new ButtonProperties("creators_end", ">>", ButtonStyle.Primary) { Disabled = page == MaxPage },
            ]);
#pragma warning restore IDE0028 // Simplify collection initialization


        [ComponentInteraction("creators_start")]
        public static InteractionCallbackProperties<MessageOptions> Start()
            => ModifyMessage(uint.MinValue);

        [ComponentInteraction("creators_back")]
        public static InteractionCallbackProperties<MessageOptions> Back(uint page)
            => ModifyMessage(page - 1);

        [ComponentInteraction("creators_next")]
        public static InteractionCallbackProperties<MessageOptions> Next(uint page)
            => ModifyMessage(page + 1);

        [ComponentInteraction("creators_end")]
